...
!bjj: todo: verify that the API model sets the parameters for IECKAI as well (because it's using IECKAI for the v and w components)
   ...
   CALL ReadVar( UI, InFile, Line, "IEC_WindType", "IEC turbulence type",ErrStat2, ErrMsg2, UnEc)
	  ...
	  ! ------------ Read in the Vave  ----------------------
   CALL ReadRVarDefault( UI, InFile, p%IEC%Vave, "IEC_Vave", "IEC Vave for Class S", UnEc, &
                         UseDefault, ErrStat2, ErrMsg2, IGNORE=(TRIM(p%IEC%IECTurbE) /= "S" ))
      CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)	

	  ! ------------ Read in the VRef  ----------------------
   CALL ReadRVarDefault( UI, InFile, p%IEC%VRef, "IEC_Vref", "IEC Vref for Class S", UnEc, &
                         UseDefault, ErrStat2, ErrMsg2, IGNORE=(TRIM(p%IEC%IECTurbE) /= "S" ))
      CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
	  
   !PRINT *,'p%IEC%IECTurbE equals'
   !PRINT *,p%IEC%IECTurbE
   !PRINT *,'p%IEC%Vave equals'
   !PRINT *,p%IEC%Vave
   !PRINT *,'p%IEC%Vref equals'
   !PRINT *,p%IEC%VRef
   !SELECT CASE ( p%IEC%IECTurbE )
	!  CASE ('S')
	!	 !PRINT *,'WE ARE IN THE CASE S'
	!	 CALL ReadVar( UI, InFile, p%IEC%Vave, "IEC_Vave", "IEC Vave for Class S",ErrStat2, ErrMsg2, UnEc)
    !        CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)	
	!	 CALL ReadVar( UI, InFile, p%IEC%VRef, "IEC_Vref", "IEC Vref for Class S",ErrStat2, ErrMsg2, UnEc)
    !        CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)	
	!	!PRINT *,'p%IEC%Vave equals'
	!	!PRINT *,p%IEC%Vave
	!	!PRINT *,'p%IEC%Vref equals'
...
CHARACTER(ErrMsgLen)         :: ErrMsg
   ...
   DO IVec=1,3
      ...
      !PRINT *,'CHECK4'
	  !PRINT *,p_grid%Z(p_grid%GridPtIndx(1))
	  !PRINT *,p_grid%HubHt
	  !PRINT *,p_grid%NPoints
	  !PRINT *,p_grid%NumGrid_Y
	  !PRINT *,FormStr5
	  ! TODO: BUG: There is a bug when writing the following line, this seems to be only related to the output writing, doesnt affect other results, therefore accept in until now!
	  ...
	  !PRINT *,'WE ARE BEHIND THE FILE HEADER'
   ...
   IF ( p%met%IsIECModel .AND. p%met%TurbModel_ID /= SpecModel_MODVKM  ) THEN
      ...
      SELECT CASE ( p%IEC%IECTurbE )
		 ...
		 CASE ('S')
            ...
            Line = Line(2:)
         CASE DEFAULT
               ! There's no number or S at the start of the string so let's move on (it's NTM).
      ...
      SELECT CASE ( TRIM( Line ) )
         ...
         CASE DEFAULT
            CALL SetErrStat( ErrID_Fatal, 'Valid entries for the IEC wind turbulence are "NTM", "xETM", "xEWM1", or "xEWM50", '// &
                             'where x is the wind turbine class (1, 2, or 3 or S).', ErrStat, ErrMsg, 'ProcessLine_IEC_WindType')
      END SELECT
      ...
      IF ( p%IEC%IEC_WindType /= IEC_NTM ) THEN
         ...
         IF (p%IEC%Vref < 0. ) THEN
            CALL SetErrStat( ErrID_Fatal, 'A wind turbine class (1, 2, or 3 or S) must be specified with the '// &
...
! calculates SigmaIEC, Lambda, IntegralScale, Lc
   ...
   IF ( p_IEC%NumTurbInp )  THEN
	  ...
	  SELECT CASE (p_IEC%IECTurbE)
		...
		CASE ( "S" )
		! TODO: This uses the same equations as in IEC ED. 3, shall the actual TI calcs go in input excel instead?
	    
		SELECT CASE ( p_IEC%IEC_WindType )
		    CASE ( IEC_NTM )
                  p_IEC%SigmaIEC(1) = p_IEC%TurbInt*( 0.75*UHub + 5.6 )                                ! [IEC-1 Ed3 6.3.1.3 (11)]
			CASE ( IEC_ETM )
			      p_IEC%SigmaIEC(1) = p_IEC%ETMc * p_IEC%TurbInt * ( 0.072 * &
                                     ( p_IEC%Vave / p_IEC%ETMc + 3.0) * (Uhub / p_IEC%ETMc - 4.0)+10.0 )       ! [IEC-1 Ed3 6.3.2.3 (19)]
	        CASE ( IEC_EWM1, IEC_EWM50, IEC_EWM100 )
                  p_IEC%SigmaIEC(1) = p_IEC%TurbInt*Uhub                                                               ! [IEC-1 Ed3 6.3.2.1 (16)]
