...
%--------------------------------------------------        
    ...
    for ic=1:3
        ...
        subplot(3,2,2*ic-1)
        hold on;
        ylabel([ comp(ic) '-comp. vel. (m/s)'])
        xlabel(  'Time (s)' )
        if ic==1
            title( ['Full-Field Comp.: ' FileName1 ' and ' FileName2], 'Interpreter','none' );
        end
        ...
        subplot(3,2,2*ic)
        hold on;
        ylabel( {'Differences in ', [ comp(ic) '-comp. vel. (m/s)'] })
        xlabel(  'Time (s)' )
        if ic==1
            title( ['Full-Field Comp.: ' FileName1 ' and ' FileName2], 'Interpreter','none' );
        end
	...
	%print('-f1',FigureFilename,'-dpdf') % then print it	
	matlab2tikz(strcat(FigureFilename,'.tex'), 'height', '\fheight', 'width', '\fwidth');
...
%     hubY_2 = [floor(ny_2/2) ceil(ny_2/2)];
    ...
    title( ['Full-Field File Comp.: ' FileName1 ' and ' FileName2], 'Interpreter','none');
    h=legend(FileName1, FileName2);
    set(h,'Interpreter','none','FontSize',4);
